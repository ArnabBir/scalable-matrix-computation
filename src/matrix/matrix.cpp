...
void add_matrix(double A[][SZ], double B[][SZ], double resultado[][SZ], int n) {
    int i, j;
    for(i=0; i< n; i++) {
...
void substract_matrix(double A[][SZ], double B[][SZ], double resultado[][SZ], int n) {
    int i, j;
    for(i=0; i< n; i++) {
        for(j=0; j<n; j++)  resultado[i][j] = A[i][j] - B[i][j];
    }
...
void strassen(double A[][SZ], double B[][SZ], double C[][SZ], int n) {
    ...
    if(n== 1) {
        ...
        double AResultant[SZ][SZ], BResultant[SZ][SZ];
...
Matrix Matrix::strassen_multiply(Matrix m2){
    ...
    double A[SZ][SZ] = {0.0}, B[SZ][SZ] = {0.0}, C[SZ][SZ] = {0.0};
    for(int i = 0; i < rows; ++i){
        for(int j = 0; j < cols; ++j){
            A[i][j] = mat[i][j];
            B[i][j] = m2.mat[i][j];
